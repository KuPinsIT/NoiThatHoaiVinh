...
class SH_Backup_class
{
	...
	function theme_options_import($file = '')
	{
		...
		$res = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."terms WHERE ".$wpdb->prefix."terms.slug = 'main-menu' OR ".$wpdb->prefix."terms.slug = 'main-menu-2' OR ".$wpdb->prefix."terms.slug = 'footer-menu'  OR ".$wpdb->prefix."terms.slug = 'top-left-menu'   OR ".$wpdb->prefix."terms.slug = 'top-right-menu'");
		//printr($res);
		if($res){
			$nav_menu = array('');
			$nav_menu['nav_menu_locations']['footer_menu'] = $res[0]->term_id;
			$nav_menu['nav_menu_locations']['main_menu'] = $res[1]->term_id;
			$nav_menu['nav_menu_locations']['main_menu_2'] = $res[2]->term_id;
			$nav_menu['nav_menu_locations']['top_left_menu'] = $res[3]->term_id;
			$nav_menu['nav_menu_locations']['top_right_menu'] = $res[4]->term_id;
			//$nav_menu['nav_menu_locations']['footer_menu'] = $res[0]->term_id;
