...
echo balanceTags($email_heading) . "\n\n";
...
switch ( $order->get_status() ) {
	case "completed" :
		echo balanceTags($order->email_order_items_table( $order->is_download_permitted(), false, true, '', '', true ));
	break;
	case "processing" :
		echo balanceTags($order->email_order_items_table( $order->is_download_permitted(), true, true, '', '', true ));
	break;
	default :
		echo balanceTags($order->email_order_items_table( $order->is_download_permitted(), true, false, '', '', true ));
	break;
}
...
if ( $totals = $order->get_order_item_totals() ) {
	foreach ( $totals as $total ) {
		echo balanceTags($total['label']) . "\t " . $total['value'] . "\n";
	}
}
