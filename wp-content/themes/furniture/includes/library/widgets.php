...
class SH_Recent_Posts extends WP_Widget
{
	...
	{
		parent::__construct( /* Base ID */'SH_Recent_Posts', /* Name */esc_html__('Blog Recent Posts ',SH_NAME), array( 'description' => esc_html__('New items with images', SH_NAME )) );
	}
...
class SH_Flickr extends WP_Widget
{
	function __construct()
	{
		parent::__construct( /* Base ID */'SH_Flickr', /* Name */esc_html__('Furniture Flickr Feed',SH_NAME), array( 'description' => esc_html__('Fetch the latest feed from Flickr', SH_NAME )) );
	}
	...
	function form($instance)
	{
		$title = ($instance) ? esc_attr($instance['title']) : esc_html__('Flicker', SH_NAME);
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('flickr_id'));?>"><?php esc_html_e('Flickr ID:', SH_NAME);?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('number'));?>"><?php esc_html_e('Number of Images:', SH_NAME);?></label>
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('number'));?>" name="<?php echo esc_attr($this->get_field_name('number'));?>" type="text" value="<?php echo esc_attr($number);?>" />
        </p>
        <?php 
	}
...
class SH_Foot_Features extends WP_Widget
{
	function __construct()
	{
		parent::__construct( /* Base ID */'SH_Foot_Features', /* Name */esc_html__('Foot Features',SH_NAME), array(      'description' => esc_html__('Foot Feature widget ', SH_NAME )) );
	}
	 ...
	 function widget($args, $instance)
	 {
		...
		<div class="media">
				<div class="media-left icon">
					...
					<img src="<?php echo esc_url($instance['logo_url']);?>" alt="<?php esc_attr($title); ?>" width="<?php echo sh_set( $img2_size, 0 ); ?>" height="<?php echo sh_set( $img2_size, 1 ); ?>">
				</div>
	 ...
	 function form($instance)
	{
		...
		$heading1 = ( $instance ) ? esc_attr($instance['heading1']) :  esc_html__('free shipping', SH_NAME);
		$heading2 = ( $instance ) ? esc_attr($instance['heading2']) : esc_html__('International', SH_NAME);?>
		
			
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('logo_url')); ?>"><?php esc_html_e('Icon Url: ', SH_NAME); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('heading1')); ?>"><?php esc_html_e('Enter Heading 1:', SH_NAME); ?></label>
		...
		<p>
            <label for="<?php echo esc_attr($this->get_field_id('heading2')); ?>"><?php esc_html_e('Enter Heading 2:', SH_NAME); ?></label>
...
class SH_About_Us extends WP_Widget
{
	function __construct()
	{
		parent::__construct( /* Base ID */'SH_About_Us', /* Name */esc_html__('About us ',SH_NAME), array(      'description' => esc_html__('About us widget ', SH_NAME )) );
	}
	 ...
	 function form($instance)
	{
		...
		$box_text = ( $instance ) ? esc_attr($instance['box_text']) :  esc_html__('Enter text', SH_NAME);
        ...
        <p>
		<p>
            <label for="<?php echo esc_attr($this->get_field_id('box_text')); ?>"><?php esc_html_e('Enter the text:', SH_NAME); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('showSocial')); ?>"><?php esc_html_e('Show Social icons', SH_NAME); ?></label>
...
class SH_feedburner extends WP_Widget
{
	...
	function __construct()
	{
		parent::__construct( /* Base ID */'SH_subscribe_mail_list', /* Name */esc_html__('Furniture Subscribe',SH_NAME), array( 'description' => esc_html__('create account on http://feedburner.com and allow users to subscribe', SH_NAME )) );
	}
		...
		<?php echo balanceTags($before_title . $title . $after_title) ; ?>
			...
			<form target="popupwindow" method="post" id="subscribe" action="http://feedburner.google.com/fb/a/mailverify" accept-charset="utf-8" class="newsletter_form">
				<div class="input-group">
					...
					<input type="email" class="form-control" name="email" value="" id="email" placeholder="<?php esc_html_e("EMAIL ADDRESS" , SH_NAME) ; ?>">
					<input type="hidden" id="uri" name="uri" value="<?php echo esc_attr($instance['ID']); ?>">
					<input type="hidden" value="en_US" name="loc">
				</div>
				<input type="submit" class="form-control" value="<?php esc_html_e("Subscribe" , SH_NAME); ?>" id="submit" name="submit">
			</form>
	...
	function form($instance)
	{
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('ID')); ?>"><?php esc_html_e('Feedburner ID:', SH_NAME); ?></label>
