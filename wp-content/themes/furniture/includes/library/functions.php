...
function sh_list_comments($comment, $args, $depth)
{
    ...
    <div id="comment-<?php comment_ID(); ?>">
		<div class="media commentBox<?php if( $comment->comment_parent ) echo ' innerComment'; ?>">
			...
			if( $email ): ?>
				<div class="media-left">
					<a href="#">
						<img src="<?php echo get_gravatar_url( $email ); ?>" alt="avatar">
					</a>
				</div>
			<?php else: ?>
				<div class="media-left">
					<a href="#">
						<img src="http://0.gravatar.com/avatar/ad516503a11cd5ca435acc9bb6523536?s=70" alt="avatar">
					</a>
				</div>
			<?php endif;?>
			
			<div class="media-body">
				<h5 class="heading"><?php echo get_comment_author_link(); ?></h5>
				<h6><?php echo get_the_date('M d, Y');?>  <span>|</span>
					<?php comment_reply_link(array_merge( $args, array('depth' => $depth, 'max_depth' => $args['max_depth'], 'reply_text' => __('Reply', SH_NAME)))) ?>   
				</h6>
...
function sh_comment_form( $args = array(), $post_id = null, $review = false )
{
	...
	$fields   =  array(
		'author' => '<div class="col-sm-4"><label for="commenterName">'. __( 'Name *', SH_NAME ).'</label><input id="name" class="form-control" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></div>',
		'email'  => '<div class="col-sm-4"><label for="commenterEmail">'. __( 'Email *', SH_NAME ).'</label><input id="email" class="form-control" name="email" ' . ( $html5 ? 'type="email"' : 'type="text"' ) . ' value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></div>',
		'url'  => '<div class="col-sm-4"><label for="commenterUrl">'. __( 'Website', SH_NAME ).'</label><input id="url" class="form-control" name="url" ' . ( $html5 ? 'type="text"' : 'type="text"' ) . ' value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30"' . $aria_req . ' /></div>',				
	);
	...
	$defaults = array(
		'fields'               => $fields,
		'comment_field'        => '<div class="row m0"><label for="comments">'. __( 'Comment', SH_NAME ).'</label><textarea id="comments" class="form-control" name="comment" cols="45" rows="8" aria-required="true"></textarea></div>',
		'must_log_in'          => '<p class="col-md-12 col-sm-12">' . sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.', SH_NAME ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'logged_in_as'         => '<p class="col-md-12 col-sm-12">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', SH_NAME ), get_edit_user_link(), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'comment_notes_before' => '<p class="col-md-12 col-sm-12">' . __( 'Your email address will not be published.', SH_NAME ) . ( $req ? $required_text : '' ) . '</p>',
		'comment_notes_after'  => '<p class="col-md-12 col-sm-12">' . sprintf( __( 'You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes: %s', SH_NAME ), ' <code>' . allowed_tags() . '</code>' ) . '</p>',
		'id_form'              => 'comments_form',
		'id_submit'            => 'submit',
		'title_reply'          => __( 'leave a comment', SH_NAME ),
	...
	?>
		<?php if ( comments_open( $post_id ) ) : ?>
			...
			<div class="row m0" id="replyForm">
					<h4 class="heading"><?php comment_form_title( $args['title_reply'], $args['title_reply_to'] ); ?> <small><?php cancel_comment_reply_link( $args['cancel_reply_link'] ); ?></small></h4>
				...
				<?php else : ?>
					<form action="<?php echo site_url( '/wp-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>" class="row"<?php echo   balanceTags($html5) ? ' novalidate' : ''; ?>>
						...
						<?php else : ?>
							...
							?>
						<?php endif; ?>
                        <?php if ( is_user_logged_in() ) : $col = 'col-lg-12 col-md-12'; else: $col = 'col-lg-12 col-md-12'; endif;?>
						<div class="<?php echo esc_attr($col);?> col-sm-12 col-xs-12">
                        <?php
						/**
						...
						<?php echo balanceTags($args['comment_notes_after']); ?>
							<button name="submit" type="submit" class="btn btn-primary btn-default filled" id="<?php echo esc_attr( $args['id_submit'] ); ?>" /><?php echo esc_attr( $args['label_submit'] ); ?></button>
